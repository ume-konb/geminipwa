# ume-konb によるフォーク版 GeminiPWA (ti-uk) の変更点・仕様（2025年5月30日時点）
...
- 設定画面に「自動挿入機能」セクションを追加しました。
    - **送信時の時刻をシスプロに追加:**
        - 有効にすると、メッセージ送信時およびリトライ時に、システムプロンプト（API送信時のみ、チャット履歴には残りません）へ現在の時刻 (`[時刻: YYYY/MM/DD HH:MM:SS]`) を自動的に追記します。
    - **送信時にダミーModelにダイスロール結果を追加:**
        - 有効にすると、メッセージ送信時およびリトライ時に、ダミーModelプロンプト（API送信時のみ）へ指定した面数のダイスロール結果 (`[ダイスロール: X面 -> Y]`) を自動的に追記します。
        - ダイスの面数は設定画面で2～100面の間で指定可能です。
        - ※ このダイスロール結果をAIの応答に連結して表示するには、設定画面の「アドバンスド」セクションにある「このダミーModelプロンプトを有効にする」と「ダミーモデルと回答を連結」の両方の項目にチェックを入れてください。

---
以下はフォーク元である titan823/geminipwa リポジトリのREADME.md (2025年5月時点のものを参考に一部編集) です。
---
...
---
以下はフォーク元リポジトリ（ona-oni/geminipwa @ 2025/05/15）のREADME.md全文です。
---
